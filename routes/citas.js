...
router.get("/", function (req, res, next) {
    conexion.query("SELECT C.id, C.fecha, Med.nombres medico, Pa.nombre paciente, Med.consultorio FROM cita_medica C, medicos Med, pacientes Pa WHERE id_medico=Med.cedula AND id_paciente=Pa.cedula;", (error, citas) => {
      if (error) {
  ...
  router.get('/agregar', (req, res) => {
    res.status(200).sendFile('form-citas.html', {root: 'public'})
  })
